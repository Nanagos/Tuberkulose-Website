...
//0
class GotoPageItem extends Item {
    constructor(pageNumber, x, y) {
        super(`gotopage${pageNumber}`, x, y);
        this.el.src = `res/0/items/${pageNumber}.png`;
        this.el.addEventListener("click", event => {
...
const pages = {
    ...
    0: {
        backgroundSrc: "res/0/imgs/background.jpeg",
        items: [
            new GotoPageItem(1, 1382, 282),
            new GotoPageItem(2, 0, 1384),
            new GotoPageItem(3, 3096, 1376)
        ]
    }
};
/*
...
function wider() {
    return background.naturalWidth / background.naturalHeight < window.innerWidth / window.innerHeight;
}
...
function hasThingAsParent(element, decide) {
    if(decide(element))
        return true;
    if(element.tagName === "HTML")
        return false;
    return hasThingAsParent(element.parentElement, decide);
};
